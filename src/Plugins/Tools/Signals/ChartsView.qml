...
Item {
    ...
    function cycleSpeed() {
        var idx = speedFactor.indexOf(speedFactorValue);
        if (idx < 0 || idx >= speedFactor.length - 1)
            speedFactorValue = speedFactor[0];
        else
            speedFactorValue = speedFactor[idx + 1];
    }
    ...
    ChartView {
        ...
        function addFactSeries(fact) {
            ...
            var color = fact.opts ? fact.opts.color : undefined;
            if (!color)
                color = Qt.rgba(1, 1, 1, 1);

            if (fact.name && fact.name.startsWith("cmd")) {
