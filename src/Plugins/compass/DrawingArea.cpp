...
// Calculate Data
    ...
    bearingDegrees2 = (bearingDegrees2 > 0.0 ? bearingDegrees2 : (360.0 + bearingDegrees2)); // correct discontinuity
...
// Draw Ellipses and DX DY point
    ...
    pen.setWidth(1);
    painter->setPen(pen);
    painter->drawEllipse(DPoint(dx, -dy), (int) (MaxRadius * dScale * kxR), (int) (MaxRadius * dScale * kyR));
...
// Draw Text DATA
    ...
    painter->drawText(QPointF(dOffset + 20, 10), QString("s") + sAxis[0] + QString().sprintf(" = %.2f",1.0/kxR));
    painter->drawText(QPointF(dOffset + 20, 20), QString("s") + sAxis[1] + QString().sprintf(" = %.2f",1.0/kyR));
...
// ERROR - COlor changing
    double maxError = 10;
    int errcolor = (abs(angError) * 255.0) / maxError;
