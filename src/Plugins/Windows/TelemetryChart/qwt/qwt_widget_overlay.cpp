...
void QwtWidgetOverlay::updateMask()
{
    ...
    if (d_data->maskMode == QwtWidgetOverlay::MaskHint) {
        ...
        // xTODO: the image doesn't need to be larger than
        ...
        QImage image(d_data->rgbaBuffer, width(), height(), qwtMaskImageFormat());
...
void QwtWidgetOverlay::paintEvent(QPaintEvent *event)
{
    ...
    if (d_data->rgbaBuffer && useRgbaBuffer) {
        const QImage image(d_data->rgbaBuffer, width(), height(), qwtMaskImageFormat());
...
void QwtWidgetOverlay::draw(QPainter *painter) const
{
    ...
    if (widget) {
        ...
        if (idx >= 0) {
            ...
            (void) QMetaObject::invokeMethod(widget,
                                             "borderPath",
                                             Qt::DirectConnection,
