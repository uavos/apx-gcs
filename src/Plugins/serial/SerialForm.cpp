...
void SerialForm::closeEvent(QCloseEvent *event)
{
  ...
  if(uart.isOpen()){
    ...
    uart.close();
    qDebug()<<tr("Serial port forwarding stopped");
  }
  ...
  switch(ui->cbTxFormat->currentIndex()){
    ...
    case 2: //List
      foreach(QString si,s.trimmed().split(',')){
        ...
        if(si.contains('"')){
          si.remove('"');
          if(si.size()) ba.append(si);
        }else ba.append((char)si.toInt());
      }
      break;
  }
...
void SerialForm::btnForward()
{
  ...
  if(!uart.open(ui->eForward->text().toUtf8().data()))return;
  ...
  qDebug()<<tr("Serial port forwarding started")+"...";
}
//==============================================================================
