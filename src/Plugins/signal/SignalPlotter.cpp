    ...
    : QWidget( parent)
{
  ...
  mHorizontalLinesColor = QColor(0x30, 0x30, 0x30);
  mHorizontalLinesCount = 4;

  mFontColor=Qt::white;//QPalette::WindowText;//=QPalette::WindowText;

  mShowAxis = false;

  mBackgroundColor = Qt::black;//QPalette::Window;//Qt::white;
  mAxisTextWidth = 0;
  mScrollOffset = 0;
  mStackBeams = false;
  mFillOpacity = 20;
...
QColor SignalPlotter::beamColor( int index ) {
  return mBeamColors[ index ];
}

void SignalPlotter::setBeamColor( int index, QColor color ) {
  if(!color.isValid()) {
...
int SignalPlotter::numBeams() {
  return mBeamColors.count();
}

void SignalPlotter::recalculateMaxMinValueForSample(const QList<double>&sampleBuf, int time )
{
  if(mStackBeams) {
    double value=0;
    ...
    if(value > 0.7*mMaxValue)
      mRescaleTime = time;
    //else if(value < 0.7*mMinValue) mRescaleTime = time;
  } else {
    double value;
    for(int i = sampleBuf.count()-1; i>= 0; i--) {
      value = sampleBuf[i];
...
void SignalPlotter::setAxisFontColor( const QColor &color )
{
  if(!color.isValid()) {
    //kDebug(1215) << "Invalid color";
    return;
...
QColor SignalPlotter::axisFontColor() const
{
  return mFontColor;
}
...
void SignalPlotter::setShowAxis( bool value )
{
...
bool SignalPlotter::showAxis() const
{
  return mShowAxis;
}

void SignalPlotter::setAxisFont( const QFont &font )
{
  mFont = font;
  mBackgroundImage = QPixmap(); //we changed a paint setting, so reset the cache
#ifdef USE_QIMAGE
...
QFont SignalPlotter::axisFont() const
{
  return mFont;
}
QString SignalPlotter::svgBackground() const {
  return mSvgFilename;
}
...
void SignalPlotter::paintEvent( QPaintEvent* event)
{
  ...
  p.fillRect(QRect(0,0,w, h),mBackgroundColor);
...
void SignalPlotter::drawWidget(QPainter *p, QRect boundingBox, bool onlyDrawPlotter)
{
  ...
  if(!onlyDrawPlotter) {
    ...
    if( mShowAxis ) {
      ...
      //else
      boundingBox.setLeft(mAxisTextWidth+2);
    }
...
void SignalPlotter::drawBackground(QPainter *p, const QRect &boundingBox)
{
...
void SignalPlotter::drawThinFrame(QPainter *p, const QRect &boundingBox)
{
...
{
  double newNiceRange = fabs(mMaxValue - mMinValue);
  /* If the range is too small we will force it to 1.0 since it
   * looks a lot nicer. */
  if ( newNiceRange < 0.0001 )
    newNiceRange = 0.01;
...
void SignalPlotter::drawVerticalLines(QPainter *p, const QRect &boundingBox, int offset)
{
...
void SignalPlotter::drawBeamToScrollableImage(int index)
{
...
void SignalPlotter::drawBeam(QPainter *p, const QRect &boundingBox, int horizontalScale, int index)
{
  QPen pen;
  pen.setWidth(2);
...
void SignalPlotter::setMaxAxisTextWidth(int axisTextWidth)
{
  mAxisTextWidth = axisTextWidth;
}

int SignalPlotter::maxAxisTextWidth() const
{
  return mAxisTextWidth;
}

void SignalPlotter::drawAxisText(QPainter *p, const QRect &boundingBox)
{
  ...
  for ( int y = 0; y < mHorizontalLinesCount +2; y++, axisTitleIndex++) {
    ...
    QString number = QString::number(value);//KGlobal::locale()->formatNumber( value, mPrecision);
...
void SignalPlotter::drawHorizontalLines(QPainter *p, const QRect &boundingBox)
{
  if(mHorizontalLinesCount <= 0) return;
...
QString SignalPlotter::lastValueAsString( int i, int precision) const
{
...
QString SignalPlotter::valueAsString( double value, int precision) const
{
  value = value / mScaleDownBy; // scale the value.  e.g. from Bytes to KB
  if(precision == -1)
    precision = (value >= 99.5)?0:((value>=0.995)?1:2);
  QString number = QString::number(value);//KGlobal::locale()->formatNumber( value, precision);
...
bool SignalPlotter::smoothGraph() const
{
  return mSmoothGraph;
}

void SignalPlotter::setSmoothGraph(bool smooth)
{
  mSmoothGraph = smooth;
#ifdef USE_QIMAGE
  mScrollableImage = QImage();
...
bool SignalPlotter::stackGraph() const
{
  return mStackBeams;
}
void SignalPlotter::setStackGraph(bool stack)
{
  mStackBeams = stack;
#ifdef USE_QIMAGE
  mScrollableImage = QImage();
...
int SignalPlotter::fillOpacity() const
{
  return mFillOpacity;
}
void SignalPlotter::setFillOpacity(int fill)
{
  mFillOpacity = fill;
#ifdef USE_QIMAGE
  mScrollableImage = QImage();
