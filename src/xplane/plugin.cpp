...
XplanePlugin::XplanePlugin(QObject * parent)
  : QObject(parent)
{
  datalink=new DatalinkServer(this);
}
//==============================================================================
//==============================================================================
//==============================================================================
...
void sendVars(void)
{
  ...
  //frame accelerations
  Vect accNED=Vect(
                -(_var_float)XPLMGetDataf(ref.az), //N=-vz
  ...
  var.gyro=Vect(
             (_var_float)XPLMGetDataf(ref.p),
             (_var_float)XPLMGetDataf(ref.q),
             (_var_float)XPLMGetDataf(ref.r)
           );//+noise_vect(2);
...
{
  printf("AP plugin version: %s\n",ASTRINGZ(VERSION));
  fflush(stdout);

  //server.tcpdebug=true;
  server.connect(NULL,TCP_PORT_SIM,false,"/sim");

  strcpy(outName, "UAVOS Autopilot SIL plugin");
  strcpy(outSig, "www.uavos.com");
  sprintf(outDesc, "Exchange data with autopilot for SIL/HIL simulation (v%s).",ASTRINGZ(VERSION));
...
float flightLoopCallback(float inElapsedSinceLastCall,float inElapsedTimeSinceLastFlightLoop,int inCounter,void * inRefcon)
{
  ...
  while (1) {
    ...
    switch(packet->data[0]){
      case 0:   //controls assignments
        ...
        for(uint i=0;i<PWM_CNT;i++){
          ...
          if(s1){
            *s1++=0;
            char *s2=index(s1,']');
            if((s2<=s1)||(s2[1]!=0))continue;
            *s2=0;
            uint aidx;
            if(sscanf(s1,"%u",&aidx)!=1)continue;
      ...
      break;
      case 1:   //PWM outputs
        if(rcnt)break; //repeated packet, skip
        memcpy(&pwm_ch,&(packet->data[1]),cnt);
        for(uint i=0;i<PWM_CNT;i++){
          if(!xpl_ref[i])continue;
          //printf("set: %s=%f\n",(char*)xpl[i],(pwm_ch[i]));
          if(xpl_is_array[i]){
