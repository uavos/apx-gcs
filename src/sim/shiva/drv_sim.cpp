...
#define NCONFA(atype, aname, asize, adescr, ...) \
    _conf_ptr_NCONF(conf, atype, aname, asize, adescr, __VA_ARGS__)
    switch (num) {
#include <config_default.h>
...
bool _drv_sim::writePacket(const uint8_t *buf, uint cnt)
{
    ...
    _tcp_client::write((uint8_t *) &sim_packet,
                       sizeof(ports->local_data.ch) + bus_packet_size_hdr + 1);
    return true;
}
//==============================================================================
...
void _drv_sim::OnEvent(evt_service &)
{
    ...
    if (bus->packet.srv.cmd != apc_loader)
        return;
    if (bus->packet_flags.broadcast)
        return;
    if (!dcnt)
        return; //jump loader
    dcnt--;
    ...
    case ldc_file: {
        if (dcnt != sizeof(_flash_file))
            break;
        memcpy(&ldr_file, data, sizeof(_flash_file));
        if (ldr_file.start_address != ldr_mem.start_address)
            break;
        if (ldr_file.size > ldr_mem.size)
            break;
...
bool _drv_sim::ldr_reply(Bus *bus, uint8_t cmd, const uint8_t *buf, uint cnt)
{
    ...
    if (buf && cnt)
        memcpy(&(bus->packet.srv.data[1]), buf, cnt);
