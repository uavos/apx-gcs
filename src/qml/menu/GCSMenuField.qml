...
Item {
    ...
    property bool bEditListText: field.fact && (field.fact.dataType==Fact.TextData || field.fact.dataType==Fact.IntData) && field.fact.enumStrings.length > 0
    ...
    property bool bIntData:      field.fact && field.fact.dataType==Fact.IntData && field.fact.enumStrings.length === 0
    ...
    Loader {
        anchors.fill: parent
        active: bQmlMenu
        source: bQmlMenu?Qt.resolvedUrl("../"+field.fact.qmlMenu):""
    }
            ...
            RowLayout {
                ...
                function createEditor()
                {
                    if(field.fieldEditor) {
                        field.fieldEditor.destroy();
                        field.fieldEditor=undefined
                    }
                    ...
                    if(bConstData || bIntData || bFloatData){
                        field.fieldEditor=fieldConstC.createObject(fieldBody);
                    }
                    if(bStatus){
                        fieldStatusC.createObject(fieldBody);
                    }
                    if(bEditText){
                        field.fieldEditor=fieldTextC.createObject(fieldBody);
                    }
                    if(bEditList || bEditListText){
                        field.fieldEditor=fieldListC.createObject(fieldBody);
                    }
                    if(bEditKey){
                        field.fieldEditor=fieldKeyC.createObject(fieldBody);
                    }
                    if(field.checkable){
                        field.fieldEditor=fieldSwitchC.createObject(fieldBody);
                    }
                    if(showNext){
                ...
                Connections {
                    target: field.fact?field.fact:fieldBody
                    ignoreUnknownSignals: true
                    onDataTypeChanged: if(fieldBody && field.fact)fieldBody.createEditor();
                }

                Component.onCompleted: {
                    createEditor();
                    /*if(field.fact){
                        field.fact.dataTypeChanged.connect(
                                    function(){
                                        if(fieldBody && field.fact)fieldBody.createEditor();
                                    } );
                    }*/
                ...
                Component {
                    ...
                    Text {
                        ...
                        //Component.onDestruction: console.log("destroyed "+field.fact.title);
                        //Component.onCompleted: console.log("created "+field.fact.title);
                    }
                }
